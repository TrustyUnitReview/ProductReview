...
public class AccountTemplate {
    ...
    /**
     * Maps an evaluated account to a jDistance based on a base account.
     * Returned as an int, representing a percentage similarity.
     */
    private int jDistCompare(Account baseAccount, Account evaluatedAccount) {
        JDistance jDistance = new JDistance();
        jDistance.setAccountA(baseAccount);
        jDistance.setAccountB(evaluatedAccount);

        return (int) jDistance.distance() * 100;
    }

    /**
     * Get top 5 closest jaccard accounts in the system
     */
    private List<Account> getTop5Jaccard(Account ownAccount) {
        List<Account> allAccounts = new ArrayList<>();
        // I'm sure there's a way to do it in one lambda, but it'll be more readable by splitting the steps
        accountRepo.findAll().forEach(allAccounts::add);
        // Exclude yourself, and any follows from list
        allAccounts.remove(ownAccount);
        ownAccount.getFollows().forEach(allAccounts::remove);
        // Sort by highest jDistance (closest) decreasing order
        allAccounts.sort(Comparator.comparingInt(acc -> jDistCompare(ownAccount, (Account) acc)).reversed());
    ...
    public String viewAccount(Model model) {
        ...
        if (account.isPresent()) {
            ...
            model.addAttribute("topJaccardList", getTop5Jaccard(selfAccount));
            return "selfAccount";
        } else {
